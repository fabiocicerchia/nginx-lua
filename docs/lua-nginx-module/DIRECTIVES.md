...
because `set $a 12` and `set $b ""` run _before_ [rewrite_by_lua](#rewrite_by_lua).
...
because `if` runs _before_ [rewrite_by_lua](#rewrite_by_lua) even if it is placed after [rewrite_by_lua](#rewrite_by_lua) in the config.
...
As with other access phase handlers, [access_by_lua](#access_by_lua) will _not_ run in subrequests.
