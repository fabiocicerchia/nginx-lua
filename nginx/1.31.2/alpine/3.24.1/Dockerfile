...
ARG NGINX_BUILD_CONFIG="\
            ...
            --add-module=/echo-nginx-module-${VER_ECHO_NGINX} \
...
COPY tpl/start-test-daemons.sh start-test-daemons.sh
...
# None of these need cleanup like lua-cjson.so/redisbloom.so do: verified by
# listing every file each package owns (`apk info -L <pkg>`) - they install
# under /usr/bin, system perl paths, etc., never under /usr/local/lib,
# /etc/nginx, /var/cache/nginx, or /usr/sbin/nginx, the only paths the final
# stage's `COPY --from=builder` pulls. The final stage is its own separate
# `FROM base` build, not a copy of the whole builder filesystem, so anything
# outside those paths simply never exists there.
...
LABEL maintainer="Fabio Cicerchia <info@fabiocicerchia.it>" \
    ...
    versions.echo-nginx=${VER_ECHO_NGINX} \
