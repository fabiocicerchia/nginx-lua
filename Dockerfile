...
LABEL maintainer="Fabio Cicerchia <info@fabiocicerchia.it>" \
    ...
    org.label-schema.description="Nginx ${VER_NGINX} with Lua support based on alpine (${ARCH}) 3.20.0." \
    org.label-schema.docker.cmd="docker run -p 80:80 -d ${DOCKER_IMAGE}:${VER_NGINX}-alpine3.20.0" \
    ...
    org.label-schema.version="${VER_NGINX}-alpine3.20.0" \
